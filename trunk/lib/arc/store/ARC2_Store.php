...
version:  2008-02-07 (Fix: Trigger results removed from all query type results for bw compatibility
                      Addition: optimizeTables method)
*/
...
class ARC2_Store extends ARC2_Class {
  ...
  function hasSetting($k) {
    ...
    return (($rs = mysql_query($sql, $this->getDBCon())) && ($row = mysql_fetch_array($rs))) ? 1 : 0;
  }
  ...
  function runQuery($infos, $type, $keep_bnode_ids = 0) {
    ...
    return $r;
    return ($type == 'ask') ? $r : array_merge($r, $trigger_r);
  }
  ...
  function getTermID($val, $term = '', $id_col = 'cid') {
    ...
    $con = $this->getDBCon();
    $sql = "SELECT " . $col . " AS id FROM " . $this->getTablePrefix() . $tbl . " WHERE BINARY val = '" . mysql_real_escape_string($val) . "' LIMIT 1";
  ...
  function unlockTables() {
    ...
    if ($err = mysql_error()) {
      $this->addError($err . ' in UNLOCK TABLES');
    }
    $this->table_lock = 0;
  }
