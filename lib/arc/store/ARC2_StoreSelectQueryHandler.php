...
version:  2008-02-25 Fix: Optimized JOIN order
*/
...
class ARC2_StoreSelectQueryHandler extends ARC2_StoreQueryHandler {
  ...
  function runQuery($infos) {
    ...
    $index_keys = array(
      ...
      'join' => array(),
      'left_join' => array(), 
      'vars' => array(), 'graph_vars' => array(), 'graph_iris' => array(), 
      'bnodes' => array(), 
    ...
    foreach ($indexes as $index) {
      ...
      $q_sql .= $q_sql ? $nl . 'UNION' . $nl : '';
      $q_sql .= $this->is_union_query ? '(' . $this->getQuerySQL() . ')' : $this->getQuerySQL();
      $this->indexes[] = $this->index;
    }
    //$q_sql .= $this->is_union_query ? $group_limit_order_sql : '';
    ...
    /* run query */
    $tmp_tbl = 'Q' . md5($q_sql . ARC2::mtime());
    //$temp = ' TEMPORARY';
    $temp = '';
    $con = $this->store->getDBCon();
    $v = $this->store->getDBVersion();
    $type_info = (($v < '04-01-00') && ($v >= '04-00-18')) ? 'ENGINE' : (($v >= '04-01-02') ? 'ENGINE' : 'TYPE');
    $type_info .= ($v < '04-01-00') ? '=MYISAM' : '=MEMORY';/* HEAP doesn't support AUTO_INCREMENT */
    $tmp_sql = 'CREATE' .$temp. ' TABLE IF NOT EXISTS ' . $tmp_tbl . ' ' . "\n" . $type_info . "\n" . $q_sql;
    $tmp = mysql_unbuffered_query($tmp_sql);
    if ($er = mysql_error()) {
      $this->addError($er);
    }
    else {
      $r = $this->getFinalQueryResult($q_sql, $vars, $tmp_tbl, $aggregate_vars);
      /* drop temp table to free file pointers */
      mysql_query('DROP' .$temp. ' TABLE IF EXISTS ' . $tmp_tbl);
      return $r;
    }
  }
  ...
  function getFinalQueryResult($q_sql, $vars, $tmp_tbl, $aggregate_vars = '') {
    ...
    $t2 = ARC2::mtime();
    $rows = array();
    $types = array(0 => 'iri', 1 => 'bnode', 2 => 'literal');
    if ($rs) {
        ...
        foreach ($vars as $var) {
          if (isset($pre_row[$var])) {
            $row[$var] = $pre_row[$var];
            $row[$var . ' type'] = isset($pre_row[$var . ' type']) ? $types[$pre_row[$var . ' type']] : (in_array($var, $aggregate_vars) ? 'literal' : 'iri');
    ...
    if ($type == 'triple') {
      ...
      foreach ($this->index['patterns'][$id]['graph_infos'] as $info) {
        if ($info['type'] == 'graph') {
          ...
          elseif ($info['iri']) {
            $val = $info['iri'];
            $this->index['graph_iris'][$val] = array_merge($this->v($val, array(), $this->index['graph_iris']), array(array('table' => $id)));
          }
        }
      }
    }
  ...
  function getGraphInfos($id) {
    ...
    if ($id) {
      ...
      /* graph */
      if ($type == 'graph') {
        $r[] = array('type' => 'graph', 'var' => $pattern['var'], 'iri' => $pattern['iri']);
      }
      ...
      if ($tbl_alias) {
        if ($var['aggregate']) {
          ...
          if (!isset($added[$var['alias']])) {
            ...
            $r .= $var['aggregate'] . '(' .$distinct_code . $tbl_alias. ') AS `' . $var['alias'] . '`';
            $added[$var['alias']] = 1;
          }
        }
        else {
    ...
    foreach ($tbl_ids as $tbl_id) {
      ...
      if ($tbl_id != $id) {
        foreach (array('s', 'p', 'o') as $tbl_term) {
          foreach (array('var', 'bnode', 'iri') as $term_type) {
  ...
  function xgetJoinSQL() {
    $r = '';
    $nl = "\n";
    foreach ($this->index['join'] as $id) {
      $sub_r = $this->getJoinConditionSQL($id);
      //$r .= $nl . 'STRAIGHT_JOIN ' . $this->getTripleTable($id) . ' T_' . $id . ' ON (' . $sub_r . $nl . ')';
      $r .= $nl . 'JOIN ' . $this->getTripleTable($id) . ' T_' . $id . ' ON (' . $sub_r . $nl . ')';
    }
    foreach (array_merge($this->index['from'], $this->index['join']) as $id) {
      $sub_r = $this->getRequiredSubJoinSQL($id);
      $r .= $sub_r ? $nl . $sub_r : '';
    }
    return $r;
  }
  
  function xgetLeftJoinSQL() {
    $r = '';
    $nl = "\n";
    foreach ($this->index['left_join'] as $id) {
      $sub_r = $this->getJoinConditionSQL($id);
      $r .= $nl . 'LEFT JOIN ' . $this->getTripleTable($id) . ' T_' . $id . ' ON (' . $sub_r . $nl . ')';
    }
    foreach ($this->index['left_join'] as $id) {
      $sub_r = $this->getRequiredSubJoinSQL($id, 'LEFT');
      $r .= $sub_r ? $nl . $sub_r : '';
    }
    return $r;
  }
  ...
  function getTriplePatternSQL($pattern, $context) {
    ...
    foreach (array('s', 'p', 'o') as $term) {
      ...
      elseif (preg_match('/literal/', $type)) {
        $term_id = $this->getTermID($pattern[$term], $term);
        $sub_r = '(T_' . $id . '.' . $term . ' = ' . $term_id . ') /* ' . preg_replace('/[\#\n]/' , ' ', $pattern[$term]) . ' */';
        if (($lang_dt = $this->v1($term . '_lang', '', $pattern)) || ($lang_dt = $this->v1($term . '_dt', '', $pattern))) {
    ...
    if ($infos = $pattern['graph_infos']) {
      ...
      $sub_r = array('graph_var' => '', 'graph_iri' => '', 'from' => '', 'from_named' => '');
      foreach ($infos as $info) {
        $type = $info['type'];
        if ($type == 'graph') {
          if ($info['iri']) {
            $term_id = $this->getTermID($info['iri'], 'g');
            $sub_r['graph_iri'] .= $sub_r['graph_iri'] ? $nl . ' AND ' : '';
            $sub_r['graph_iri'] .= '(' .$tbl_alias. ' = ' . $term_id . ') /* ' . str_replace('#' , '::', $info['iri']) . ' */';
          }
        }
      }
  ...
  function getFilterPatternSQL($pattern, $context) {
    ...
    if ($this->hasUnconnectedFilterVars($id)) {
      return 'FALSE';
    }
  ...
  function detectExpressionValueType($pattern_ids) {
    foreach ($pattern_ids as $id) {
      ...
      if ($type == 'literal') {
        $dt = $pattern['dt'];
        if (in_array($dt, array($this->xsd . 'integer', $this->xsd . 'float', $this->xsd . 'double'))) {
          return 'numeric';
        }
      }
    }
  ...
  function getRelationalExpressionSQL($pattern, $context, $val_type = '', $parent_type = '') {
    ...
    foreach ($pattern['patterns'] as $sub_id) {
      ...
      $sub_type = preg_match('/literal/', $sub_type) ? 'literal' : $sub_type;
  ...
  function getIriExpressionSQL($pattern, $context, $val_type = '') {
    $val = $pattern['iri'];
    $r = $pattern['operator'];
    $r .= is_numeric($val) ? ' ' . $val : ' "' .mysql_real_escape_string($val). '"';
    return $r;
  }
  ...
  function getLiteralExpressionSQL($pattern, $context, $val_type = '', $parent_type = '') {
    $val = $pattern['val'];
    $r = $pattern['operator'];
    if (is_numeric($val) && $this->v('dt', 0, $pattern)) {
      $r .= ' ' . $val;
    }
    elseif (preg_match('/^(true|false)$/i', $val) && ($this->v1('dt', '', $pattern) == 'http://www.w3.org/2001/XMLSchema#boolean')) {
      $r .= ' ' . strtoupper($val);
    }
    elseif ($parent_type == 'regex') {
      $sub_r = mysql_real_escape_string($val);
      $r .= ' "' . preg_replace('/\x5c\x5c/', '\\', $sub_r) . '"';
    }
    else {
      $r .= ' "' . mysql_real_escape_string($val) . '"';
    }
    if (($lang_dt = $this->v1('lang', '', $pattern)) || ($lang_dt = $this->v1('dt', '', $pattern))) {
  ...
  function getFunctionExpressionSQL($pattern, $context, $val_type = '', $parent_type = '') {
    ...
    if ($this->allow_extension_functions) {
      /* mysql functions */
      if (preg_match('/^http\:\/\/web\-semantics\.org\/ns\/mysql\/(.*)$/', $fnc_iri, $m)) {
  ...
  function getFunctionCallSQL($pattern, $context) {
    $f_iri = $pattern['iri'];
    if (preg_match('/(integer|double|float|string)$/', $f_iri)) {/* skip conversions */
      $sub_pattern = $pattern['args'][0];
      $sub_type = $sub_pattern['type'];
      $m = 'get' . ucfirst($sub_type) . 'ExpressionSQL';
      if (method_exists($this, $m)) {
  ...
  function getRegexCallSQL($pattern, $context) {
    ...
    if ($ac >= 2) {
      ...
      return ($sub_r_1 && $sub_r_2) ? $sub_r_1 . ' REGEXP ' . $opt . $sub_r_2 : '';
    }
    return '';
  }
  ...
  function getLIMITSQL() {
    ...
    if ($limit != -1) {
      $offset = ($offset == -1) ? 0 : mysql_real_escape_string($offset);
      $r = 'LIMIT ' . $offset . ',' . $limit; 
    }
    elseif ($offset != -1) {
      $r = 'LIMIT ' . mysql_real_escape_string($offset) . ',999999999999'; /* mysql doesn't support stand-alone offsets .. */
    }
    return $r ? $nl . $r : '';
  }
  ...
  function getValueSQL($q_tbl) {
    ...
    foreach ($vars as $var) {
      ...
      if ($var['aggregate']) {
        ...
        if (in_array($col, array('s', 'o'))) {
          ...
          //$r .= ', ' . $nl . '    CASE TMP.`' . $var_name . ' type` WHEN 2 THEN "literal" WHEN 1 THEN "bnode" ELSE "iri" END AS `' . $var_name . ' type`';
        }
        $vc++;
        if ($col == 'o') {
    ...
    if ($this->v('order_infos', array(), $this->infos['query'])) {
      $rs = mysql_query("ALTER TABLE " .$q_tbl. " ADD _pos_ INT NOT NULL AUTO_INCREMENT PRIMARY KEY");
      if ($er = mysql_error()) {
        $this->addError($er);
      }
      $r .= $nl . ' ORDER BY _pos_';
