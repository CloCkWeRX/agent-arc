  ...
  function changeNamespaceURI($old_uri, $new_uri) {
    ...
    if ($this->store->lockTables()) {
      ...
      foreach (array('id', 's', 'o') as $id_col) {
        ...
        $sql = 'SELECT id, val FROM ' . $tbl . ' WHERE val LIKE "' . mysql_real_escape_string($old_uri). '%"';
        if ($rs = mysql_query($sql)) {
          while ($row = mysql_fetch_array($rs)) {
            $new_val = str_replace($old_uri, $new_uri, $row['val']);
            $new_id = $this->store->getTermID($new_val, $id_col , 'id');
            if (!$new_id) {/* new ns uri */
              $sub_sql = "UPDATE " . $tbl . " SET val = '" . mysql_real_escape_string($new_val) . "' WHERE id = " . $row['id'];
              $sub_r = mysql_query($sub_sql);
              $id_changes++;
            }
            else {
